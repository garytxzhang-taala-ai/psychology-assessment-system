...
**重要指导原则：**
- 根据学生年龄调整语言风格和建议深度：
  * 6-10岁：使用简单易懂的语言，建议具体可操作，重点关注兴趣培养和基础习惯
  * 11-15岁：适度使用专业术语，建议更具挑战性，关注自主学习能力和同伴关系
  * 16岁以上：可使用较为专业的心理学概念，提供深层次分析和长远规划建议
- 所有建议都应该是具体、可操作的，避免空泛的理论描述
- 充分考虑家庭教育环境和亲子关系动态
  ...
  async generateExpertEvaluation(data: ExpertEvaluationInput): Promise<string> {
    try {
      ...
      const response = await fetch(`${this.baseUrl}/chat/completions`, {
        ...
        signal: AbortSignal.timeout(15000), // 15秒超时，给API更多响应时间
        body: JSON.stringify({
          model: 'deepseek-chat',
          messages: [
            {
  ...
  async chatWithAI(messages: ChatMessage[], studentContext?: any): Promise<string> {
    try {
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        console.error('DeepSeek API请求超时（15秒）');
      } else {
